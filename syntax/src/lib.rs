...
use common::{ErrorKind, Errors, Loc, NO_LOC};

// save a little typing than writing "Default::default()"
pub(crate) fn dft<T: Default>() -> T {
    T::default()
}

pub(crate) fn mk_stmt(loc: Loc, kind: StmtKind) -> Stmt {
    Stmt { loc, kind }
}

pub(crate) fn mk_expr(loc: Loc, kind: ExprKind) -> Expr {
    Expr {
        loc,
        ty: dft(),
        kind,
...
impl<T> VecExt for Vec<T> {
    ...
    fn pushed(mut self, i: Self::Item) -> Self {
        (self.push(i), self).1
    }
