...
use common::{BinOp, ErrorKind, Errors, Loc, UnOp, NO_LOC};
...
impl<'p> Parser<'p> {
    ...
    fn class_list(l: Vec<&'p ClassDef<'p>>, r: &'p ClassDef<'p>) -> Vec<&'p ClassDef<'p>> {
        l.pushed(r)
    }
    #[rule(ClassList -> ClassDef)]
    fn class_list1(c: &'p ClassDef<'p>) -> Vec<&'p ClassDef<'p>> {
        vec![c]
    }
    ...
    fn field_list_v(l: Vec<FieldDef<'p>>, r: &'p VarDef<'p>, _s: Token) -> Vec<FieldDef<'p>> {
        l.pushed(r.into())
    }
    #[rule(FieldList -> FieldList FuncDef)]
    fn field_list_f(l: Vec<FieldDef<'p>>, r: &'p FuncDef<'p>) -> Vec<FieldDef<'p>> {
        l.pushed(r.into())
    }
    #[rule(FieldList ->)]
    ...
    fn var_def_list(l: Vec<&'p VarDef<'p>>, _c: Token, r: &'p VarDef<'p>) -> Vec<&'p VarDef<'p>> {
        l.pushed(r)
    }
    #[rule(VarDefList -> VarDef)]
    fn var_def_list1(v: &'p VarDef<'p>) -> Vec<&'p VarDef<'p>> {
        vec![v]
    }
    ...
    fn stmt_list(l: Vec<Stmt<'p>>, r: Stmt<'p>) -> Vec<Stmt<'p>> {
        l.pushed(r)
    }
    #[rule(StmtList ->)]
    ...
    fn stmt_return1(r: Token, expr: Expr<'p>, _s: Token) -> Stmt<'p> {
        mk_stmt(r.loc(), Some(expr).into())
    }
    #[rule(Stmt -> Return Semi)]
    fn stmt_return0(r: Token, _s: Token) -> Stmt<'p> {
        mk_stmt(r.loc(), None.into())
    }
    #[rule(Stmt -> Print LPar ExprList RPar Semi)]
    fn stmt_print(p: Token, _l: Token, print: Vec<Expr<'p>>, _r: Token, _s: Token) -> Stmt<'p> {
        mk_stmt(p.loc(), print.into())
    }
    ...
    fn maybe_else1(_e: Token, b: Stmt<'p>) -> Option<Block<'p>> {
        Some(mk_block(b))
    }
    #[rule(MaybeElse ->)]
    #[prec(Empty)]
    ...
    fn simple_assign(dst: Expr<'p>, a: Token, src: Expr<'p>) -> Stmt<'p> {
        mk_stmt(a.loc(), Assign { dst, src }.into())
    }
    #[rule(Simple -> VarDef)] // the VarDef without init
    fn simple_var_def(v: &'p VarDef<'p>) -> Stmt<'p> {
        mk_stmt(v.loc, v.into())
    }
    ...
    fn simple_var_det_init_refernce(
        &self,
        _v: Token,
        name: Token,
        a: Token,
    ...
    */
    ...
    //try_1
    /*
    #[rule(Expr -> FuncSel LPar ExprListOrEmpty RPar)]
    fn expr_call(func: Expr<'p>, l: Token, arg: Vec<Expr<'p>>, _r: Token) -> Expr<'p> {
      mk_expr(l.loc(), Call { func: Box::new(func), arg, func_ref: dft() }.into())
    }
    #[rule(FuncSel -> Expr)]
    fn funcsel(func: Expr<'p>) -> Expr<'p> { func }
    */
    //try_2
    /*
    #[rule(Expr -> LValue)]
    ...
    */
    ...
    #[rule(Expr -> True)]
    fn expr_true(t: Token) -> Expr<'p> {
        mk_expr(t.loc(), true.into())
    }
    #[rule(Expr -> False)]
    fn expr_false(f: Token) -> Expr<'p> {
    ...
    fn expr_paren(_l: Token, m: Expr<'p>, _r: Token) -> Expr<'p> {
        m
    }
    #[rule(Expr -> Expr Add Expr)]
    fn expr_add(l: Expr<'p>, op: Token, r: Expr<'p>) -> Expr<'p> {
        mk_bin(l, r, op.loc(), BinOp::Add)
    }
    #[rule(Expr -> Expr Sub Expr)]
    fn expr_sub(l: Expr<'p>, op: Token, r: Expr<'p>) -> Expr<'p> {
        mk_bin(l, r, op.loc(), BinOp::Sub)
    }
    #[rule(Expr -> Expr Mul Expr)]
    fn expr_mul(l: Expr<'p>, op: Token, r: Expr<'p>) -> Expr<'p> {
        mk_bin(l, r, op.loc(), BinOp::Mul)
    }
    #[rule(Expr -> Expr Div Expr)]
    fn expr_div(l: Expr<'p>, op: Token, r: Expr<'p>) -> Expr<'p> {
        mk_bin(l, r, op.loc(), BinOp::Div)
    }
    #[rule(Expr -> Expr Mod Expr)]
    fn expr_mod(l: Expr<'p>, op: Token, r: Expr<'p>) -> Expr<'p> {
        mk_bin(l, r, op.loc(), BinOp::Mod)
    }
    #[rule(Expr -> Expr Eq Expr)]
    fn expr_eq(l: Expr<'p>, op: Token, r: Expr<'p>) -> Expr<'p> {
        mk_bin(l, r, op.loc(), BinOp::Eq)
    }
    #[rule(Expr -> Expr Ne Expr)]
    fn expr_ne(l: Expr<'p>, op: Token, r: Expr<'p>) -> Expr<'p> {
        mk_bin(l, r, op.loc(), BinOp::Ne)
    }
    #[rule(Expr -> Expr Lt Expr)]
    fn expr_lt(l: Expr<'p>, op: Token, r: Expr<'p>) -> Expr<'p> {
        mk_bin(l, r, op.loc(), BinOp::Lt)
    }
    #[rule(Expr -> Expr Le Expr)]
    fn expr_le(l: Expr<'p>, op: Token, r: Expr<'p>) -> Expr<'p> {
        mk_bin(l, r, op.loc(), BinOp::Le)
    }
    #[rule(Expr -> Expr Ge Expr)]
    fn expr_ge(l: Expr<'p>, op: Token, r: Expr<'p>) -> Expr<'p> {
        mk_bin(l, r, op.loc(), BinOp::Ge)
    }
    #[rule(Expr -> Expr Gt Expr)]
    fn expr_gt(l: Expr<'p>, op: Token, r: Expr<'p>) -> Expr<'p> {
        mk_bin(l, r, op.loc(), BinOp::Gt)
    }
    #[rule(Expr -> Expr And Expr)]
    fn expr_and(l: Expr<'p>, op: Token, r: Expr<'p>) -> Expr<'p> {
        mk_bin(l, r, op.loc(), BinOp::And)
    }
    #[rule(Expr -> Expr Or Expr)]
    fn expr_or(l: Expr<'p>, op: Token, r: Expr<'p>) -> Expr<'p> {
        mk_bin(l, r, op.loc(), BinOp::Or)
    }
    #[rule(Expr -> ReadInteger LPar RPar)]
    fn expr_read_int(r: Token, _l: Token, _r: Token) -> Expr<'p> {
        mk_expr(r.loc(), ReadInt.into())
    }
    #[rule(Expr -> ReadLine LPar RPar)]
    fn expr_read_line(r: Token, _l: Token, _r: Token) -> Expr<'p> {
        mk_expr(r.loc(), ReadLine.into())
    }
    #[rule(Expr -> This)]
    fn expr_this(t: Token) -> Expr<'p> {
    ...
    fn expr_list(l: Vec<Expr<'p>>, _c: Token, r: Expr<'p>) -> Vec<Expr<'p>> {
        l.pushed(r)
    }
    #[rule(ExprList -> Expr)]
    fn expr_list1(e: Expr<'p>) -> Vec<Expr<'p>> {
        vec![e]
    }

    #[rule(ExprListOrEmpty -> ExprList)]
    fn expr_list_or_empty1(e: Vec<Expr<'p>>) -> Vec<Expr<'p>> {
        e
    }
    ...
    fn maybe_owner1(e: Expr<'p>, _d: Token) -> Option<Box<Expr<'p>>> {
        Some(Box::new(e))
    }
    #[rule(MaybeOwner ->)]
    fn maybe_owner0() -> Option<Box<Expr<'p>>> {
        None
    }
    ...
    /*
    #[rule(VarSel -> Expr)]
    fn var_sel1(e: Expr<'p>) -> Expr<'p> { e }
    */

    #[rule(LValue -> VarSel)]
    fn lvalue_var_sel(e: Expr<'p>) -> Expr<'p> {
        e
    }
    ...
    fn type_listorempty1() -> Vec<SynTy<'p>> {
        vec![]
    }
    #[rule(TypeList -> TypeList Comma Type)]
    fn type_list1(param: Vec<SynTy<'p>>, _c: Token, t: SynTy<'p>) -> Vec<SynTy<'p>> {
        param.pushed(t)
    }
    #[rule(TypeList -> Type)]
