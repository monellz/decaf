...
impl<'a> Ty<'a> {
    ...
    pub const fn error() -> Ty<'a> {
        Ty {
            arr: 0,
            kind: TyKind::Error,
        }
    }
    pub const fn null() -> Ty<'a> {
        Ty {
            arr: 0,
            kind: TyKind::Null,
        }
    }
    pub const fn int() -> Ty<'a> {
        Ty {
            arr: 0,
            kind: TyKind::Int,
        }
    }
    pub const fn bool() -> Ty<'a> {
        Ty {
            arr: 0,
            kind: TyKind::Bool,
        }
    }
    pub const fn void() -> Ty<'a> {
        Ty {
            arr: 0,
            kind: TyKind::Void,
        }
    }
    pub const fn string() -> Ty<'a> {
        Ty {
            arr: 0,
            kind: TyKind::String,
        }
    ...
    pub fn mk_obj(c: &'a ClassDef<'a>) -> Ty<'a> {
        Ty {
            arr: 0,
            kind: TyKind::Object(Ref(c)),
        }
    }
    pub fn mk_class(c: &'a ClassDef<'a>) -> Ty<'a> {
        Ty {
            arr: 0,
    ...
    pub fn mk_func(f: &'a FuncDef<'a>) -> Ty<'a> {
        Ty {
            arr: 0,
            kind: TyKind::Func(f.ret_param_ty.get().unwrap()),
        }
    }
    ...
    pub fn is_func(&self) -> bool {
        self.arr == 0
            && if let TyKind::Func(_) = self.kind {
                true
            } else {
                false
            }
    ...
    pub fn is_class(&self) -> bool {
        self.arr == 0
            && if let TyKind::Class(_) = self.kind {
                true
            } else {
                false
            }
    ...
    pub fn is_object(&self) -> bool {
        self.arr == 0
            && if let TyKind::Object(_) = self.kind {
                true
            } else {
                false
            }
...
impl fmt::Debug for Ty<'_> {
    fn fmt(&self, f: &mut fmt::Formatter) -> Result<(), fmt::Error> {
        match &self.kind {
            ...
            TyKind::Func(ret_param) => show_func_ty(
                ret_param[1..].iter().cloned(),
                ret_param[0],
                self.is_arr(),
                f,
            ),
...
pub fn show_func_ty<'a>(
    mut param: impl Iterator<Item = Ty<'a>>,
    ret: Ty,
    is_arr: bool,
    f: &mut fmt::Formatter,
) -> Result<(), fmt::Error> {
    if is_arr {
        write!(f, "(")?;
    } // [] have higher priority than T => T, so add () here, make it (T => T)[]
