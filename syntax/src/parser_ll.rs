...
use crate::{ast::*, check_str, dft, mk_block, mk_expr, mk_int_lit, mk_stmt, ty::*, VecExt};
use common::{BinOp, ErrorKind, Errors, HashMap, HashSet, Loc, UnOp, NO_LOC};
use parser_macros::ll1;

pub fn work<'p>(
    code: &'p str,
    alloc: &'p ASTAlloc<'p>,
) -> Result<&'p Program<'p>, Errors<'p, Ty<'p>>> {
...
impl<'p> Parser<'p> {
    fn error(&mut self, token: &Token<'p>, lexer_loc: Loc) {
        ...
        match token.ty {
            TokenKind::_Err => {
                if self.error.0.last().map(|x| x.0) != Some(loc) {
                    self.error
                        .issue(loc, ErrorKind::UnrecognizedChar(token.piece[0] as char))
                }
            }
    ...
    fn _parse<'l: 'p>(
        ...
        let (prod, rhs) = if let Some(x) = table.get(&(lookahead.ty as u32)) {
            x
        } else {
...
impl<'p> Parser<'p> {
    ...
    fn program(&self, class: Vec<&'p ClassDef<'p>>) -> &'p Program<'p> {
        self.alloc.program.alloc(Program {
            class: class.reversed(),
            main: dft(),
            scope: dft(),
        })
    }
    ...
    fn class_list(l: &'p ClassDef<'p>, r: Vec<&'p ClassDef<'p>>) -> Vec<&'p ClassDef<'p>> {
        r.pushed(l)
    }
    #[rule(ClassList ->)]
    fn class_list1() -> Vec<&'p ClassDef<'p>> {
        vec![]
    }
    ...
    fn maybe_extends1(_e: Token, name: Token) -> Option<&'p str> {
        Some(name.str())
    }
    #[rule(MaybeExtends ->)]
    fn maybe_extends0() -> Option<&'p str> {
        None
    }

    #[rule(FieldList -> FieldDef FieldList)]
    fn field_list(l: FieldDef<'p>, r: Vec<FieldDef<'p>>) -> Vec<FieldDef<'p>> {
        r.pushed(l)
    }
    #[rule(FieldList ->)]
    fn field_list0() -> Vec<FieldDef<'p>> {
        vec![]
    }
    ...
    fn filed_def_fv(
        ...
        syn_ty: SynTy<'p>,
        name: Token,
        fov: Option<(Vec<&'p VarDef<'p>>, Block<'p>)>,
    ) -> FieldDef<'p> {
        let (loc, name) = (name.loc(), name.str());
        if let Some((param, body)) = fov {
            ...
            FieldDef::VarDef(self.alloc.var.alloc(VarDef {
                loc,
                name,
                syn_ty: Some(syn_ty),
                init: None,
    ...
    fn func_or_var_f(
        ...
        _r: Token,
        body: Block<'p>,
    ) -> Option<(Vec<&'p VarDef<'p>>, Block<'p>)> {
        Some((param, body))
    }
    #[rule(FuncOrVar -> Semi)]
    fn func_or_var_v(_s: Token) -> Option<(Vec<&'p VarDef<'p>>, Block<'p>)> {
        None
    }

    #[rule(VarDefListOrEmpty -> VarDefList)]
    fn var_def_list_or_empty1(l: Vec<&'p VarDef<'p>>) -> Vec<&'p VarDef<'p>> {
        l
    }
    #[rule(VarDefListOrEmpty ->)]
    fn var_def_list_or_empty0() -> Vec<&'p VarDef<'p>> {
        vec![]
    }
    #[rule(VarDefList -> VarDef VarDefListRem)]
    fn var_def_list(l: &'p VarDef<'p>, r: Vec<&'p VarDef<'p>>) -> Vec<&'p VarDef<'p>> {
        r.pushed(l)
    }
    ...
    #[rule(VarDefListRem ->)]
    fn var_def_list_rem0() -> Vec<&'p VarDef<'p>> {
        vec![]
    }

    // the logic of ExprList is completely the same as VarDefList...
    #[rule(ExprListOrEmpty -> ExprList)]
    fn expr_list_or_empty1(l: Vec<Expr<'p>>) -> Vec<Expr<'p>> {
        l
    }
    #[rule(ExprListOrEmpty ->)]
    fn expr_list_or_empty0() -> Vec<Expr<'p>> {
        vec![]
    }
    #[rule(ExprList -> Expr ExprListRem)]
    fn expr_list(l: Expr<'p>, r: Vec<Expr<'p>>) -> Vec<Expr<'p>> {
        r.pushed(l)
    }
    #[rule(ExprListRem -> Comma Expr ExprListRem)]
    fn expr_list_rem(_c: Token, l: Expr<'p>, r: Vec<Expr<'p>>) -> Vec<Expr<'p>> {
        r.pushed(l)
    }
    #[rule(ExprListRem ->)]
    fn expr_list_rem0() -> Vec<Expr<'p>> {
        vec![]
    }
    ...
    fn stmt_list(l: Stmt<'p>, r: Vec<Stmt<'p>>) -> Vec<Stmt<'p>> {
        r.pushed(l)
    }
    #[rule(StmtList ->)]
    fn stmt_list0() -> Vec<Stmt<'p>> {
        vec![]
    }

    #[rule(Stmt -> Simple Semi)]
    fn stmt_simple(s: Stmt<'p>, _s: Token) -> Stmt<'p> {
        s
    }
    ...
    fn stmt_if(
        ...
        _r: Token,
        on_true: Stmt<'p>,
        on_false: Option<Block<'p>>,
    ) -> Stmt<'p> {
        mk_stmt(
            i.loc(),
            Box::new(If {
                cond,
                on_true: mk_block(on_true),
                on_false,
            })
            .into(),
        )
    ...
    fn stmt_return(r: Token, expr: Option<Expr<'p>>, _s: Token) -> Stmt<'p> {
        mk_stmt(r.loc(), expr.into())
    }
    #[rule(Stmt -> Print LPar ExprList RPar Semi)]
    fn stmt_print(p: Token, _l: Token, print: Vec<Expr<'p>>, _r: Token, _s: Token) -> Stmt<'p> {
        mk_stmt(p.loc(), print.reversed().into())
    }
    #[rule(Stmt -> Break Semi)]
    fn stmt_break(b: Token, _s: Token) -> Stmt<'p> {
        mk_stmt(b.loc(), Break.into())
    }
    #[rule(Stmt -> Block)]
    fn stmt_block(b: Block<'p>) -> Stmt<'p> {
        mk_stmt(b.loc, b.into())
    }
    ...
    fn maybe_assign1(a: Token, src: Expr<'p>) -> Option<(Loc, Expr<'p>)> {
        Some((a.loc(), src))
    }
    #[rule(MaybeAssign ->)]
    fn maybe_assign0() -> Option<(Loc, Expr<'p>)> {
        None
    }
    ...
    fn maybe_else1(_e: Token, b: Block<'p>) -> Option<Block<'p>> {
        Some(b)
    }
    #[rule(MaybeElse ->)]
    fn maybe_else0() -> Option<Block<'p>> {
        None
    }

    #[rule(MaybeExpr -> Expr)]
    fn maybe_expr1(e: Expr<'p>) -> Option<Expr<'p>> {
        Some(e)
    }
    #[rule(MaybeExpr ->)]
    fn maybe_expr0() -> Option<Expr<'p>> {
        None
    }

    #[rule(Op1 -> Or)]
    fn op1(o: Token) -> (Loc, BinOp) {
        (o.loc(), BinOp::Or)
    }

    #[rule(Op2 -> And)]
    fn op2(a: Token) -> (Loc, BinOp) {
        (a.loc(), BinOp::And)
    }

    #[rule(Op3 -> Eq)]
    fn op3_eq(e: Token) -> (Loc, BinOp) {
        (e.loc(), BinOp::Eq)
    }
    #[rule(Op3 -> Ne)]
    fn op3_ne(n: Token) -> (Loc, BinOp) {
        (n.loc(), BinOp::Ne)
    }

    #[rule(Op4 -> Lt)]
    fn op4_lt(l: Token) -> (Loc, BinOp) {
        (l.loc(), BinOp::Lt)
    }
    #[rule(Op4 -> Le)]
    fn op4_le(l: Token) -> (Loc, BinOp) {
        (l.loc(), BinOp::Le)
    }
    #[rule(Op4 -> Ge)]
    fn op4_ge(g: Token) -> (Loc, BinOp) {
        (g.loc(), BinOp::Ge)
    }
    #[rule(Op4 -> Gt)]
    fn op4_gt(g: Token) -> (Loc, BinOp) {
        (g.loc(), BinOp::Gt)
    }

    #[rule(Op5 -> Add)]
    fn op5_add(a: Token) -> (Loc, BinOp) {
        (a.loc(), BinOp::Add)
    }
    #[rule(Op5 -> Sub)]
    fn op5_sub(s: Token) -> (Loc, BinOp) {
        (s.loc(), BinOp::Sub)
    }

    #[rule(Op6 -> Mul)]
    fn op6_add(m: Token) -> (Loc, BinOp) {
        (m.loc(), BinOp::Mul)
    }
    #[rule(Op6 -> Div)]
    fn op6_div(d: Token) -> (Loc, BinOp) {
        (d.loc(), BinOp::Div)
    }
    #[rule(Op6 -> Mod)]
    fn op6_mod(m: Token) -> (Loc, BinOp) {
        (m.loc(), BinOp::Mod)
    }

    #[rule(Op7 -> Sub)]
    fn op7_neg(n: Token) -> (Loc, UnOp) {
        (n.loc(), UnOp::Neg)
    }
    #[rule(Op7 -> Not)]
    fn op7_not(n: Token) -> (Loc, UnOp) {
        (n.loc(), UnOp::Not)
    }
    ...
    */
    ...
    fn expr1(l: Expr<'p>, ts: Terms<'p>) -> Expr<'p> {
        merge_terms(l, ts)
    }
    #[rule(Term1 -> Op1 Expr2 Term1)] // or
    fn term1(o: (Loc, BinOp), l: Expr<'p>, r: Terms<'p>) -> Terms<'p> {
        r.pushed((l, o))
    }
    #[rule(Term1 ->)]
    fn term1_0() -> Terms<'p> {
    ...
    fn expr2(l: Expr<'p>, ts: Terms<'p>) -> Expr<'p> {
        merge_terms(l, ts)
    }
    #[rule(Term2 -> Op2 Expr3 Term2)] // and
    fn term2(o: (Loc, BinOp), l: Expr<'p>, r: Terms<'p>) -> Terms<'p> {
        r.pushed((l, o))
    }
    #[rule(Term2 ->)]
    fn term2_0() -> Terms<'p> {
    ...
    fn expr3(l: Expr<'p>, ts: Terms<'p>) -> Expr<'p> {
        merge_terms(l, ts)
    }
    #[rule(Term3 -> Op3 Expr4 Term3)] // eq, ne
    fn term3(o: (Loc, BinOp), l: Expr<'p>, r: Terms<'p>) -> Terms<'p> {
        r.pushed((l, o))
    }
    #[rule(Term3 ->)]
    fn term3_0() -> Terms<'p> {
    ...
    fn expr4(l: Expr<'p>, ts: Terms<'p>) -> Expr<'p> {
        merge_terms(l, ts)
    }
    #[rule(Term4 -> Op4 Expr5 Term4)] // lt, le, ge, gt
    fn term4(o: (Loc, BinOp), l: Expr<'p>, r: Terms<'p>) -> Terms<'p> {
        r.pushed((l, o))
    }
    #[rule(Term4 ->)]
    fn term4_0() -> Terms<'p> {
    ...
    fn expr5(l: Expr<'p>, ts: Terms<'p>) -> Expr<'p> {
        merge_terms(l, ts)
    }
    #[rule(Term5 -> Op5 Expr6 Term5)] // add sub
    fn term5(o: (Loc, BinOp), l: Expr<'p>, r: Terms<'p>) -> Terms<'p> {
        r.pushed((l, o))
    }
    #[rule(Term5 ->)]
    fn term5_0() -> Terms<'p> {
    ...
    fn expr6(l: Expr<'p>, ts: Terms<'p>) -> Expr<'p> {
        merge_terms(l, ts)
    }
    #[rule(Term6 -> Op6 Expr7 Term6)] // mul, div, mod
    fn term6(o: (Loc, BinOp), l: Expr<'p>, r: Terms<'p>) -> Terms<'p> {
        r.pushed((l, o))
    }
    #[rule(Term6 ->)]
    fn term6_0() -> Terms<'p> {
    ...
    fn expr8(l: Expr<'p>, ts: Vec<IndexOrIdOrCall<'p>>) -> Expr<'p> {
        merge_idx_id_call(l, ts)
    }
    ...
    */
    ...
    #[rule(Expr9 -> True)]
    fn expr9_true(t: Token) -> Expr<'p> {
        mk_expr(t.loc(), true.into())
    }
    #[rule(Expr9 -> False)]
    fn expr9_false(f: Token) -> Expr<'p> {
        mk_expr(f.loc(), false.into())
    }
    ...
    #[rule(Expr9 -> Null)]
    fn expr9_null(n: Token) -> Expr<'p> {
        mk_expr(n.loc(), NullLit.into())
    }
    #[rule(Expr9 -> ReadInteger LPar RPar)]
    fn expr9_read_integer(r: Token, _l: Token, _r: Token) -> Expr<'p> {
        mk_expr(r.loc(), ReadInt.into())
    }
    #[rule(Expr9 -> ReadLine LPar RPar)]
    fn expr9_read_line(r: Token, _l: Token, _r: Token) -> Expr<'p> {
        mk_expr(r.loc(), ReadLine.into())
    }
    #[rule(Expr9 -> This)]
    fn expr9_this(t: Token) -> Expr<'p> {
        mk_expr(t.loc(), This.into())
    }
    ...
    */
    ...
    fn expr9_new(n: Token, noa: NewClassOrArray<'p>) -> Expr<'p> {
        ...
        match noa {
            ...
            NewClassOrArray::NewArray(elem, len) => mk_expr(
                loc,
                NewArray {
                    elem,
                    len: Box::new(len),
    ...
    fn new_array_rem(_r: Token, l: Token, mut dim_len: (u32, Expr<'p>)) -> (u32, Expr<'p>) {
        (dim_len.0 += 1, dim_len).1
    }
    ...
    */
    ...
    fn type_string(s: Token) -> SynTy<'p> {
        SynTy {
            loc: s.loc(),
            arr: 0,
            kind: SynTyKind::String,
        }
    }
    #[rule(SimpleType -> Class Id)]
    fn type_class(c: Token, name: Token) -> SynTy<'p> {
        SynTy {
            loc: c.loc(),
            arr: 0,
