...
impl<'a> SymbolPass<'a> {
    pub fn program(&mut self, p: &'a Program<'a>) {
        self.scoped(ScopeOwner::Global(p), |s| {
            ...
            fn calc_order<'a>(
                c: &'a ClassDef<'a>,
                inherit_order: &mut HashMap<Ref<'a, ClassDef<'a>>, u32>,
            ) -> u32 {
                ...
                if let Some(p) = c.parent {
                    match s.scopes.lookup_class(p) {
                        Some(p) => {
                            ...
                            if calc_order(c, &mut inherit_order)
                                <= calc_order(p, &mut inherit_order)
                            {
                                return s.errors.issue(c.loc, CyclicInheritance);
                            }
                        }
                        None => {
                            return s.errors.issue(c.loc, NoSuchClass(p));
                        }
                    }
                }
            }
    ...
    fn stmt(&mut self, s: &'a Stmt<'a>) {
        match &s.kind {
            ...
            StmtKind::For(f) => self.scoped(ScopeOwner::Local(&f.body), |s| {
                if let StmtKind::LocalVarDef(v) = &f.init.kind {
                    s.var_def(v);
                }
...
impl<'a> SymbolPass<'a> {
            ...
            self.scoped(ScopeOwner::Class(p), |s| {
                c.scope.borrow_mut().retain(|name, sym| {
                    ...
                    if let Some((p_sym, _)) = s.scopes.lookup(name, true) {
                        match (sym, p_sym) {
                            (Symbol::Func(_), Symbol::Var(_))
                            | (Symbol::Var(_), Symbol::Func(_)) => s.errors.issue(
                                sym.loc(),
                                ConflictDeclaration {
                                    prev: p_sym.loc(),
                                    name,
                            ...
                            (Symbol::Func(f), Symbol::Func(pf)) => {
                                if f.static_ || pf.static_ {
                                    s.errors
                                        .issue(f.loc, ConflictDeclaration { prev: pf.loc, name })
                                } else if !f.ret_ty().assignable_to(pf.ret_ty())
                                    || f.param.len() != pf.param.len()
                                    || !f
                                        .param
                                        .iter()
                                        .zip(pf.param.iter())
                                        .all(|(f, pf)| pf.ty.get().assignable_to(f.ty.get()))
                                {
                                    s.errors.issue(
                                        f.loc,
                                        OverrideMismatch {
                            ...
                            (Symbol::Var(_), Symbol::Var(_)) => {
                                s.errors.issue(sym.loc(), OverrideVar(name))
                            }
    ...
    fn check_main(&mut self, p: &Program) {
        match p.main.get() {
            Some(main)
                if match main.scope.borrow().get(MAIN_METHOD) {
