...
impl<'a> TypePass<'a> {
        ...
        match &s.kind {
            StmtKind::Assign(a) => {
                ...
                if l.is_func() || !r.assignable_to(l) {
                    self.errors
                        .issue(s.loc, IncompatibleBinary { l, op: "=", r })
                }
                false
            }
            StmtKind::LocalVarDef(v) => {
                ...
                if let Some((loc, e)) = &v.init {
                    ...
                    if !r.assignable_to(l) {
                        self.errors
                            .issue(*loc, IncompatibleBinary { l, op: "=", r })
                    }
                }
            ...
            StmtKind::If(i) => {
                ...
                let s2 = if let Some(of) = &i.on_false {
                    self.block(of)
                } else {
                    false
                };
            ...
            StmtKind::Break(_) => {
                if self.loop_cnt == 0 {
                    self.errors.issue(s.loc, BreakOutOfLoop)
                }
                false
            }
        ...
        let ty = match &e.kind {
            ...
            This(_) => {
                if !self.cur_func.unwrap().static_ {
                    ...
                    self.errors.issue(e.loc, ThisInStatic)
                }
            }
    ...
    fn unary(&mut self, u: &'a Unary<'a>, loc: Loc) -> Ty<'a> {
        ...
        match u.op {
            UnOp::Neg => {
                if r != Ty::int() && r != Ty::error() {
                    self.errors.issue(loc, IncompatibleUnary { op: "-", r })
                }
                Ty::int()
            }
            UnOp::Not => {
                if r != Ty::bool() && r != Ty::error() {
                    self.errors.issue(loc, IncompatibleUnary { op: "!", r })
                }
                Ty::bool()
            }
        ...
        match &v.owner {
            Some(owner) => {
                ...
                if v.name == LENGTH && owner.is_arr() {
                    if !c.arg.is_empty() {
                        self.errors
                            .issue(loc, LengthWithArgument(c.arg.len() as u32))
                    }
                    return Ty::int();
                }
                match owner.kind {
                    TyKind::Class(cl) | TyKind::Object(cl) => {
                        if let Some(symbol) = cl.lookup(v.name) {
