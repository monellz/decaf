use common::Ref;
use tac::{CallKind, FuncNameKind, Intrinsic::_Halt, Operand, Tac, TacFunc, TacIter, TacKind};
...
impl<'a> BB<'a> {
    ...
    pub fn prev_with_entry<'b>(
        &'b self,
        this: usize,
    ) -> (usize, impl IntoIterator<Item = usize> + Clone + 'b) {
        (
            this,
            self.prev
                .iter()
                .map(|x| *x as usize + 1)
                .chain(if this == 1 { Some(0) } else { None }),
        )
    }
}
...
impl<'a> FuncBB<'a> {
    ...
    pub fn new<'b>(f: &'b TacFunc<'a>) -> Option<FuncBB<'a>> {
        ...
        while let Some(first) = iter {
                ...
                match cur {
                    ...
                    Some(cur1) => {
                        match cur1.payload.borrow().kind {
                            ...
                            TacKind::Jif { label, z, cond } => {
                                break match cond[0] {
                                    Operand::Const(c) => {
                                        if (c == 0) == z {
                                            // (Jz, and is z) or (Jnz and is not z), do the jump
                                            has_label = true;
                                            NextKind::Jmp(label)
                                        } else {
                                            NextKind::Jmp(bb.len() as u32 + 1)
                                        }
                                    }
                                    Operand::Reg(r) => {
                                        has_label = true;
            ...
            if has_label {
                label_to_map.push(bb.len() as u32);
            }
        ...
        bb.push(BB {
            ...
            first: None,
            last: None,
            next: NextKind::Ret(None),
            prev: vec![],
        });
        for unfill in label_to_map {
            match &mut bb[unfill as usize].next {
                NextKind::Jmp(jump) | NextKind::Jif { jump, .. } => {
                    *jump = label2bb[*jump as usize]
                }
                _ => {}
            }
        }
    ...
    pub fn to_tac_func(&self) -> TacFunc<'a> {
        ...
        for (idx, b) in self.bb.iter().enumerate() {
            ...
            match b.next {
                ...
                NextKind::Jmp(jump) => {
                    if jump != idx as u32 + 1 {
                        f.push(TacKind::Jmp { label: jump });
                    }
                }
                NextKind::Jif { cond, z, jump, .. } => {
                    f.push(TacKind::Jif {
                        label: jump,
                        z,
...
pub(crate) fn checked_simplify<'a>(
    mut bb: Vec<BB<'a>>,
    f: Option<&TacFunc>,
) -> Option<Vec<BB<'a>>> {
    fn dfs(x: usize, bb: &mut [BB], vis: &mut [bool]) {
        ...
        bb[x]
            ...
            .iter()
            .filter_map(|&x| x)
            .for_each(|x| dfs(x as usize, bb, vis));
    }
    ...
    for (_, mut b) in bb.into_iter().enumerate().filter(|(idx, _)| vis[*idx]) {
        b.next = match b.next {
            ...
            NextKind::Jif {
                ...
                cond,
                z,
                fail: actual[fail as usize],
                jump: actual[jump as usize],
            },
            n => n,
        };
    ...
    for idx in 0..new.len() {
        ...
        new[idx]
            ...
            .iter()
            .filter_map(|&x| x)
            .for_each(|x| new[x as usize].prev.push(idx as u32));
    }
