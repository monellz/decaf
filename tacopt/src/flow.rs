...
impl<M: Meet<u32>> Flow<M> {
    ...
    pub fn solve<I: IntoIterator<Item = usize>>(
        &mut self,
        graph: impl IntoIterator<Item = (usize, I)> + Clone,
    ) {
        ...
        while changed {
            ...
            for (this, edges) in graph.clone() {
                ...
                for edge in edges {
                    let off1 = edge * each;
                    for (x, y) in in_[off..off + each]
                        .iter_mut()
                        .zip(out[off1..off1 + each].iter())
                    {
                        *x = M::meet(*x, *y);
                    }
                }
    ...
    pub fn get(&mut self, idx: usize) -> FlowElem {
        ...
        FlowElem {
            gen: &mut gen[idx..idx + each],
            kill: &mut kill[idx..idx + each],
            in_: &mut in_[idx..idx + each],
            out: &mut out[idx..idx + each],
        }
    }
