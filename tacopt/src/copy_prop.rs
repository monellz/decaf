...
pub fn work(f: &mut FuncBB) {
    ...
    let reg2copy = reg2copy
        .iter()
        .map(|(k, v)| (*k, iter2bs(v, copy2id.len())))
        .collect();
    ...
    for (off, b) in f.bb.iter().enumerate().map(|b| ((b.0 + 1) * each, b.1)) {
        compute_gen_kill(
            b,
            &mut gen[off..off + each],
            &mut kill[off..off + each],
            &reg2copy,
            &copy2id,
        );
    }
...
fn do_optimize(
    ...
    fn lookup(reg: u32, in_: &[u32], copy2id: &IndexSet<(u32, u32)>) -> u32 {
        for (id, &(dst, src)) in copy2id.iter().enumerate() {
            if in_.bsget(id) && reg == dst {
                // propagate, allow cascading multi-level copy in one pass of optimization
    ...
    for t in b.iter() {
        ...
        tac.rw_mut().0.iter_mut().for_each(|r| {
