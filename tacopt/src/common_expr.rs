...
use bitset::traits::*;
use common::{BinOp, HashMap, HashSet, Ref, UnOp};
use tac::{CallKind, MemHint, Operand, Tac, TacIter, TacKind, TacPayload};

pub fn work(f: &mut FuncBB) {
    WorkCtx::new(f).work(f);
}
...
impl<'a> WorkCtx<'a> {
    pub fn new(f: &FuncBB<'a>) -> WorkCtx<'a> {
        let (mut write2id, mut rhs2id, mut tac2id) =
        ...
        let write2id = write2id
            .iter()
            .map(|(&k, v)| (k, iter2bs(v, rhs2id.len())))
            .collect();
        WorkCtx {
            write2id,
            rhs2id,
    ...
    pub fn work(&mut self, f: &mut FuncBB<'a>) {
        ...
        for (off, b) in
            f.bb.iter()
                .enumerate()
                .map(|(idx, bb)| ((idx + 1) * each, bb))
        {
            self.compute_gen_kill(b, &mut gen[off..off + each], &mut kill[off..off + each]);
        }
        for x in out.iter_mut().skip(each) {
            *x = !0;
        }
        available_expr_flow.solve(
            f.bb.iter()
                .enumerate()
                .map(|b| b.1.prev_with_entry(b.0 + 1)),
        );
    ...
    fn compute_gen_kill(&self, b: &BB, gen: &mut [u32], kill: &mut [u32]) {
        for t in b.iter() {
            let payload = t.payload.borrow();
            let payload = &*payload;
            if let Some(rhs) = TacRhs::from_tac(payload.kind).map(|rhs| self.rhs2id[&rhs]) {
                gen.bsset(rhs)
            }
    ...
    fn dfs(
        ...
        idx: usize,
        f: &mut FuncBB<'a>,
        iter: impl IntoIterator<Item = &'a Tac<'a>>,
        rhs: u32,
        new: u32,
    ) {
        if self.vis[idx] {
            ...
            if self.tac2id.get(&Ref(t)) == Some(&rhs) {
                let old = std::mem::replace(
                    t.payload
                        ...
                        .rw_mut()
                        .1
                        .expect("This tac with rhs must also have a lhs."),
                    new,
                );
                let payload = TacPayload {
                    kind: TacKind::Assign {
                        dst: old,
                        src: [Operand::Reg(new)],
                    },
        ...
        for (t_idx, t) in f.bb[idx].iter().enumerate() {
            ...
            if let Some(rhs) = TacRhs::from_tac(payload.kind) {
                ...
                if in_.bsget(rhs) {
                    ...
                    let prev = TacIter::new(f.bb[idx].first, Some(t), t_idx + 1)
                        .rev()
                        .skip(1);
                    ...
                    let w = payload
                        ...
                        .rw()
                        .1
                        .expect("The tac with rhs must also have a lhs.");
                    payload.kind = TacKind::Assign {
                        dst: w,
                        src: [Operand::Reg(new)],
                    };
                }
            }
            if let Some(rhs) = TacRhs::from_tac(old_kind).map(|rhs| self.rhs2id[&rhs]) {
                in_.bsset(rhs)
            }
            if let Some(w) = old_kind.rw().1.and_then(|w| self.write2id.get(&w)) {
                in_.bsandn(w)
            }
