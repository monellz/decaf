...
use tac::{CallKind, MemHint, Operand, Tac, TacIter, TacNode};
...
impl<'a> WorkCtx<'a> {
    ...
    fn compute_gen_kill(&self, b: &BB, gen: &mut [u32], kill: &mut [u32]) {
        for t in b.iter() {
            let tac = t.tac.get();
            if let Some(rhs) = TacRhs::from_tac(tac).map(|rhs| self.rhs2id[&rhs]) {
                gen.bsset(rhs)
            }
    ...
    fn dfs(
        ...
        idx: usize,
        f: &mut FuncBB<'a>,
        iter: impl IntoIterator<Item = &'a TacNode<'a>>,
        rhs: u32,
        new: u32,
    ) {
        if self.vis[idx] {
        ...
        for t in iter {
            if self.tac2id.get(&Ref(t)) == Some(&rhs) {
                ...
                let copy = f.alloc.alloc(TacNode {
                    tac: Tac::Assign {
                        dst,
                        src: [Operand::Reg(new)],
                    }
