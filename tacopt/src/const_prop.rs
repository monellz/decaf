use crate::bb::{simplify, FuncBB, NextKind};
use tac::{Operand, Tac};
...
fn transfer(tac: Tac, env: &mut [Value]) {
    ...
    match tac {
        ...
        Un { op, dst, r } => {
            env[dst as usize] = match r[0] {
                Const(r) => C(op.eval(r)),
                Reg(r) => match env[r as usize] {
                    C(r) => C(op.eval(r)),
                    r => r,
                },
            }
        ...
        Assign { dst, src } => {
            env[dst as usize] = match src[0] {
                Const(r) => C(r),
                Reg(r) => env[r as usize],
            }
        }
        Call { dst, .. } => {
            if let Some(dst) = dst {
                env[dst as usize] = Nac
            }
        }
    ...
    for (idx, b) in f.bb.iter_mut().enumerate() {
        ...
        for t in b.iter() {
            ...
            for r in tac.rw_mut().0 {
                if let Operand::Reg(r1) = *r {
                    if let Value::Const(r1) = env[r1 as usize] {
                        *r = Operand::Const(r1);
                    }
                }
            }
        ...
        match &mut b.next {
            ...
            &mut NextKind::Jif {
                ...
                if let Value::Const(c) = env[cond as usize] {
                    b.next = if (c == 0) == z {
                        NextKind::Jmp(jump)
                    } else {
                        NextKind::Jmp(fail)
