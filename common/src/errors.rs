...
impl<Ty: fmt::Debug> fmt::Debug for ErrorKind<'_, Ty> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        match self {
            ...
            ConflictDeclaration { prev, name } => write!(
                f,
                "declaration of '{}' here conflicts with earlier declaration at {:?}",
                name, prev
            ),
            ...
            OverrideMismatch { func, p } => write!(
                f,
                "overriding method '{}' doesn't match the type signature in class '{}'",
                func, p
            ),
            IncompatibleUnary { op, r } => write!(f, "incompatible operand: {} {:?}", op, r),
            IncompatibleBinary { l, op, r } => {
                write!(f, "incompatible operands: {:?} {} {:?}", l, op, r)
            }
            ...
            RefInStatic { field, func } => write!(
                f,
                "can not reference a non-static field '{}' from static method '{}'",
                field, func
            ),
            BadFieldAccess { name, owner } => {
                write!(f, "cannot access field '{}' from '{:?}'", name, owner)
            }
            PrivateFieldAccess { name, owner } => {
                write!(f, "field '{}' of '{:?}' not accessible here", name, owner)
            }
            NoSuchField { name, owner } => write!(f, "field '{}' not found in '{:?}'", name, owner),
            LengthWithArgument(cnt) => write!(
                f,
                "function 'length' expects 0 argument(s) but {} given",
                cnt
            ),
            NotFunc { name, owner } => {
                write!(f, "'{}' is not a method in class '{:?}'", name, owner)
            }
            ArgcMismatch {
                name,
                expect,
            ...
            ArgMismatch { loc, arg, param } => write!(
                f,
            ...
            BadPrintArg { loc, ty } => write!(
                f,
                "incompatible argument {}: {:?} given, int/bool/string expected",
                loc, ty
            ),
            ReturnMismatch { expect, actual } => write!(
                f,
