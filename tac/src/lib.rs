...
use common::{BinOp, IndexSet, UnOp};
use std::{
    cell::{Cell, RefCell},
    fmt,
};
...
impl<'a> TacFunc<'a> {
    ...
    pub fn iter(&self) -> TacIter<'a> {
        TacIter::new(
            self.first_last.map(|(f, _)| f),
            self.first_last.map(|(_, l)| l),
            self.tac_len as usize,
        )
    }
}
...
impl TacKind {
    ...
    pub fn rw(&self) -> (&[Operand], Option<u32>) {
        ...
        match self {
            ...
            Un { dst, r, .. } | Assign { dst, src: r } | Load { dst, base: r, .. } => {
                (r, Some(*dst))
            }
            Param { src } => (src, None),
            Call { dst, kind } => (
                if let CallKind::Virtual(fp, _) = kind {
                    fp
                } else {
                    &[]
                },
    ...
    pub fn rw_mut(&mut self) -> (&mut [Operand], Option<&mut u32>) {
        ...
        match self {
            ...
            Ret { src } => (
                src.as_mut().map(|src| src.as_mut()).unwrap_or(&mut []),
                None,
            ),
            Jmp { .. } | Label { .. } => (&mut [], None),
            Jif { cond, .. } => (cond, None),
            Store { src_base, .. } => (src_base, None),
            LoadInt { dst, .. } | LoadStr { dst, .. } | LoadVTbl { dst, .. } => {
                (&mut [], Some(dst))
            }
        }
    }
...
impl Intrinsic {
    ...
    pub fn has_ret(self) -> bool {
        use Intrinsic::*;
        match self {
            _Alloc | _ReadLine | _ReadInt | _StringEqual => true,
            _PrintInt | _PrintString | _PrintBool | _Halt => false,
        }
    }
}
