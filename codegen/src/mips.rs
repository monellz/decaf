...
pub mod regs {
    ...
    pub const NAME: [&str; 32] = [
        "$v0", "$v1", "$a0", "$a1", "$a2", "$a3", "$t0", "$t1", "$t2", "$t3", "$t4", "$t5", "$t6",
        "$t7", "$t8", "$t9", "$s0", "$s1", "$s2", "$s3", "$s4", "$s5", "$s6", "$s7", "$fp", "$ra",
        "$zero", "$at", "$k0", "$k1", "$gp", "$sp",
    ];
...
impl AsmTemplate {
    ...
    pub fn rw(&self, r: &mut Vec<Reg>, w: &mut Vec<Reg>) {
        ...
        match *self {
            ...
            Jal(_) | Jalr(_) => {
                for a in regs::ARG {
                    r.push(Reg::PreColored(a as u32));
                }
                for crs in regs::CALLER_SAVE {
                    w.push(Reg::PreColored(crs as u32));
                }
            ...
            J(_) | Label(_) => {}
            Ret => {
                for ces in regs::CALLEE_SAVE {
                    r.push(Reg::PreColored(ces as u32));
                }
    ...
    pub fn imm_mut(&mut self) -> Option<&mut Imm> {
        use AsmTemplate::*;
        match self {
            BinI(_, _, _, i) | Lw(_, _, i) | Sw(_, _, i) | Li(_, i) => Some(i),
            _ => None,
        }
    }
