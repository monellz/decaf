...
use bitset::traits::*;
use common::{BinOp, HashMap, HashSet};
use tac::{CallKind, Intrinsic, Operand, Tac, TacProgram};
use tacopt::{
    bb::{FuncBB, NextKind},
    flow::{Flow, FlowElem, Or},
};
...
impl AllocCtx for FuncGen<'_, '_> {
    ...
    fn initial(&self) -> (Vec<u32>, Vec<Node>) {
        ...
        (
            (REG_N..self.reg_num + REG_N).collect(),
            (0..self.reg_num + REG_N)
                .map(|r| {
                    if r < REG_N {
...
impl<'a: 'b, 'b> FuncGen<'a, 'b> {
    pub fn work(f: &'b FuncBB<'a>, p: &'b TacProgram<'a>, m: AllocMethod) -> Vec<AsmTemplate> {
        ...
        let mut fu = FuncGen {
            param_num: f.param_num,
            reg_num: f.reg_num,
            ch_param_num: 0,
            name: &f.name,
            program: p,
            bb: Vec::new(),
            spill2slot: HashMap::new(),
        };
        fu.populate(f);
        match m {
            AllocMethod::Graph => Allocator::work(&mut fu),
...
impl<'a: 'b, 'b> FuncGen<'a, 'b> {
    ...
    pub(crate) fn find_spill_slot(&mut self, vreg: u32) -> Imm {
        let vreg = vreg - REG_N;
        if vreg < self.param_num.max(ARG_N) {
            // function arguments already have places to spill
            Imm::Tag(vreg)
        } else {
...
impl FuncGen<'_, '_> {
    fn select_inst(&mut self, t: Tac, b: &mut Vec<AsmTemplate>, arg_num: &mut u32) {
        use AsmTemplate::*;
        match t {
            Tac::Bin { op, dst, lr } => match lr {
                [Operand::Const(l), Operand::Const(r)] => {
                    b.push(Li(vreg(dst), Imm::Int(op.eval(l, r))))
                }
                [Operand::Reg(l), Operand::Const(r)] => {
                    b.push(BinI(op, vreg(dst), vreg(l), Imm::Int(r)))
                }
                [Operand::Const(l), Operand::Reg(r)] => {
            ...
            Tac::Store { src_base, off, .. } => {
                let (src, base) = (
                    self.build_operand(src_base[0], b),
                    self.build_operand(src_base[1], b),
                );
                b.push(Sw(src, base, Imm::Int(off)));
            }
            Tac::LoadStr { dst, s } => b.push(AsmTemplate::La(vreg(dst), format!("_STRING{}", s))),
            Tac::LoadVTbl { dst, v } => b.push(AsmTemplate::La(
                vreg(dst),
                format!("_{}", self.program.vtbl[v as usize].class),
            )),
            Tac::LoadFunc { dst, f } => b.push(AsmTemplate::La(
                vreg(dst),
                self.program.func[f as usize].name.clone(),
            )),
            Tac::Label { .. } | Tac::Ret { .. } | Tac::Jmp { .. } | Tac::Jif { .. } => {
                unreachable!("Shouldn't meet Ret/Jmp/Jif/Label in a tac bb.")
            }
        }
    }
    ...
    fn build_next(
        ...
        match next {
            ...
            NextKind::Ret(src) => {
                ...
                if idx + 2 != epilogue {
                    // + 2, 1 for "prologue takes index 0", 1 for next bb should inc by 1 naturally
            ...
            NextKind::Jif {
                ...
                b.push(AsmTemplate::B(
                    format!("{}_L{}", self.name, jump + 1),
                    vreg(cond),
                    z,
                ));
