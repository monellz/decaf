...
use tac::{CallKind, FuncNameKind, Intrinsic, Operand, TacKind, TacProgram};
use tacopt::{
    bb::{FuncBB, NextKind},
...
impl AllocCtx for FuncGen<'_, '_> {
    ...
    fn initial(&self) -> (Vec<u32>, Vec<Node>) {
        ...
        (
            (REG_N..self.max_reg + REG_N).collect(),
            (0..self.max_reg + REG_N)
                .map(|r| {
                    if r < REG_N {
...
impl FuncGen<'_, '_> {
    fn select_inst(&mut self, t: TacKind, b: &mut Vec<AsmTemplate>, arg_cnt: &mut u32) {
        use AsmTemplate::*;
        match t {
            TacKind::Bin { op, dst, lr } => match lr {
            ...
            TacKind::LoadStr { dst, s } => {
                b.push(AsmTemplate::La(vreg(dst), format!("_STRING{}", s)))
            }
            TacKind::LoadVTbl { dst, v } => b.push(AsmTemplate::La(
                vreg(dst),
            ...
            | TacKind::Ret { .. }
            | TacKind::Jmp { .. }
            | TacKind::Jif { .. } => unreachable!("Shouldn't meet Ret/Jmp/Jif/Label in a tac bb."),
        }
    }
