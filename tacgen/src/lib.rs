...
use crate::info::*;
use ::tac::{self, Intrinsic::*, Operand::*, TacKind, TacKind::*, *};
        ...
        for &c in &p.class {
            for f in &c.field {
                if let FieldDef::FuncDef(fu) = f {
                    ...
                    self.label_cnt = 0;
                    let mut f = TacFunc::empty(
                        alloc,
                        if Ref(c) == Ref(p.main.get().unwrap()) && fu.name == MAIN_METHOD {
                            FuncNameKind::Main
                        } else {
                            FuncNameKind::Member {
                                class: c.name,
    ...
    match &e.kind {
      ...
      VarSel(v) => {
        ...
        match var.owner.get().unwrap() {
          ScopeOwner::Local(_) | ScopeOwner::Param(_) | ScopeOwner::LambdaParam(_) | ScopeOwner::LambdaExprLocal(_) => if let Some(src) = assign { // `off` is register
