use crate::{Alloc, CompileCfg, Parser, Stage};
use colored::*;
use std::{
    any::Any,
    fmt,
...
pub fn test_all(path: impl AsRef<Path>, pa: Pa) -> io::Result<Vec<TestResult>> {
    ...
    let mut files = fs::read_dir(path)?
        .filter_map(|f| {
            ...
            if path.is_file() && name.ends_with(".decaf") {
                Some(name.to_owned())
            } else {
                None
            }
    ...
    let ret = files
        ...
        .map(|f| {
            test_one_caught(
                path.join(f),
                out.join(f).with_extension("result"),
                ans.join(f).with_extension("result"),
                pa,
            )
        })
        .collect();
...
pub fn test_one_caught(
    ...
    panic::set_hook(Box::new(move |panic_info| {
        if let Some(l) = panic_info.location() {
            *loc1.lock().unwrap() = Some(PanicLoc {
    ...
    let ret = panic::catch_unwind(panic::AssertUnwindSafe(|| test_one(&i, &o, &ans, pa)))
        .unwrap_or_else(|e| {
            TestResult::new(
                ...
                o,
                ans,
                ResultKind::RuntimeError(PanicInfo {
                    payload: get_payload(e),
                    loc: loc.lock().unwrap().clone(),
                }),
            )
        });
...
pub fn test_one(
    ...
    let kind = run(&i, &o, pa)
        ...
        .map_or_else(ResultKind::IOError, |(out, ans)| {
            ResultKind::new(&out, &ans, ignore_line)
        });
    ...
    let out = match crate::compile(&fs::read_to_string(i)?, &Alloc::default(), cfg) {
        Ok(p) => match cfg.stage {
            ...
            Stage::Asm => {
                ...
                Command::new(SPIM_PATH)
                    .arg("-file")
                    .arg(o.with_extension("s"))
                    .stdout(Stdio::from(File::create(&o)?))
                    .spawn()?
                    .wait()?;
                fs::read_to_string(o)?
            }
        ...
        match &self.kind {
            ...
            ResultKind::Fail {
                ...
                writeln!(
                    ...
                    "{}: {}",
                    "Fail".red(),
                    format!("first different line on {}", first_diff).yellow()
                )?;
                writeln!(
                    f,
                    "{}",
