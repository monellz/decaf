...
use std::fmt::Write;
use tac::{CallKind, Tac, TacProgram};
...
pub fn write_tac(t: Tac, pr: &TacProgram, p: &mut IndentPrinter) {
    ...
    match t {
        ...
        Ret { src } => {
            if let Some(src) = src {
                write!(p, "return {:?}", src[0])
            } else {
                write!(p, "return")
            }
        }
        Jmp { label } => write!(p, "branch %{}", label),
        Jif { label, z, cond } => write!(
            p,
            "if ({:?} {} 0) branch %{}",
            cond[0],
            if z { "==" } else { "!=" },
        ...
        LoadStr { dst, s } => write!(
            ...
            "%{} = \"{}\"",
            dst,
            pr.str_pool.get_index(s as usize).unwrap()
        ),
